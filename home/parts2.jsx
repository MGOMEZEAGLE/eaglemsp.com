...
function WhyEagle({ onCta }) {
  const cards = [
    {
      icon: 'users', title: 'Your one IT person is stretched thin',
      body: 'One person cannot cover help desk, security, patching, and strategy, and they cannot take a vacation without their phone lighting up. Eagle backs them up, or takes it on entirely, for one predictable monthly cost.',
    },
    {
      icon: 'lock', title: 'Your cyber insurance renewal is getting harder',
      body: 'The questionnaires keep getting longer and the required controls keep growing. We close the gaps, document the proof, and help you answer honestly so your policy holds up when you need it.',
    },
    {
      icon: 'trending', title: 'A neighboring organization got hit with ransomware',
      body: 'A town down the road, a business in your industry. It rattles you, and it should. We put layered defenses and 24/7 monitoring in place so you are not the next easy target.',
    },
  ];
  return (
...
function SocialProof() {
  const stats = [
    { num: '97%', lbl: 'Customer satisfaction score' },
  ...
  const reviews = [
    ...
    { placeholder: true, quote: 'Quote pending from a non-profit or manufacturing client.', name: 'Client name to follow', role: 'Non-profit or manufacturer · NH or ME', initials: '★' },
  ];
  return (
    <section className="section navy social">
      <div className="container">
        ...
        <div className="proof-reviews">
          {reviews.map((r) => (
            <figure className={`proof-card reveal${r.placeholder ? ' is-slot' : ''}`} key={r.name}>
              {r.placeholder
                ? <div className="slot-tag">Third testimonial slot</div>
                : <div className="stars">{[0, 1, 2, 3, 4].map((s) => <Icon2 key={s} name="star" />)}</div>}
              <blockquote>{r.placeholder ? r.quote : `“${r.quote}”`}</blockquote>
          ...
          {!hasEmbed && (
            <div className="smileback-fallback">
              <div className="sb-score">
                ...
                <div className="sb-lbl">Positive customer satisfaction score</div>
              </div>
