...
/* Featured videos — short CEO-hosted security awareness clips.
   To publish a video: add its embed URL as `src` (YouTube/Vimeo embed link).
   Without a src the card shows a branded play panel with the same copy. */
function FeaturedVideos() {
  const videos = [
    {
      cat: 'Vendor & Partner Risk',
      title: 'Dirty Partners',
      body: "A trusted business partner's email gets hacked, and the bad actor uses that relationship to slip past your guard. This video breaks down how attackers exploit the trust between partners and what to watch for before you click.",
      len: '3 min',
      src: '',
    },
    {
      ...
      title: 'MFA',
      body: "Multi-factor authentication stops most attacks, but it's not foolproof. This video shows how attackers try to get around it, and the simple habits that keep your accounts protected anyway.",
      len: '3 min',
      src: '',
    },
    {
      cat: 'Business Email Compromise',
      title: 'Fake CFO Email',
      body: "An email that looks like it's from your CFO, asking for an urgent wire transfer. This video walks through a real-world example of executive impersonation and how to spot the red flags before it costs you.",
      len: '4 min',
      src: '',
    },
  ];
  return (
    <section className="section vid-section" id="videos">
      <div className="container">
        <div className="sec-head reveal">
          <span className="eyebrow">Straight from our CEO</span>
          <h2>Two minutes that could save your business.</h2>
          <p>Short, plain-English videos on the attacks we actually see hitting New England organizations. No jargon, no fear-mongering, just what to watch for.</p>
        </div>
        <div className="vid-grid">
          {videos.map((v, i) => (
            <article className="vid-card reveal" key={v.title} style={{ transitionDelay: `${i * 0.06}s` }}>
              <div className="vid-frame">
                {v.src
                  ? <iframe src={v.src} title={v.title} loading="lazy" allow="accelerometer; clipboard-write; encrypted-media; picture-in-picture" allowFullScreen></iframe>
                  : <div className="vid-poster" role="img" aria-label={`Video: ${v.title}`}>
                      <span className="vp-play"><Icon3 name="play" /></span>
                      <span className="vp-len">{v.len}</span>
                    </div>}
              </div>
        ...
        <div className="vid-foot reveal">
          <p>New videos added as the threats change. Want them sent to your team?</p>
          <a className="link-arrow" href="Contact.html">Ask about security awareness training <Icon3 name="arrow" /></a>
        </div>
      </div>
    </section>
  );
...
Object.assign(window, { LocalSupport, FeaturedVideos, BlogGrid, CtaBand, Footer, ContactModal });
