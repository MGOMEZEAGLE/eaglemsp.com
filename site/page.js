...
(function () {
  ...
  /* Brand-safe illustrated panel used wherever real photography is not yet
     supplied. Cool navy/mist only. `src` renders a real photo when available. */
  function imgph(label, icon, cls, src) {
    ...
    return '<figure class="imgph illus ' + (cls || '') + '" role="img" aria-label="' + esc(label) + '">' +
      '<span class="illus-art">' +
        '<span class="illus-ring r1"></span><span class="illus-ring r2"></span><span class="illus-ring r3"></span>' +
        '<span class="illus-ico">' + svg(icon) + '</span>' +
      '</span>' +
      '<figcaption class="illus-cap">' + esc(label) + '</figcaption>' +
    '</figure>';
  }
  ...
  /* Single place to set Eagle's TimeZest booking link. TimeZest blocks being
     framed inside other sites, so booking opens in a new tab instead. */
  var TIMEZEST_URL = window.EAGLE_TIMEZEST_URL || "https://eaglemsp.timezest.com/";

  function schedule(s) {
    var url = s.url || TIMEZEST_URL;
    return '<section class="section"><div class="container">' + (s.title ? secHead(s, true) : '') +
      '<div class="schedule-embed schedule-cta reveal" style="max-width:760px;margin:0 auto">' +
        '<div class="se-ico">' + svg("calendar") + '</div>' +
        '<h3>' + esc(s.heading || "Book time with an Eagle strategist") + '</h3>' +
        '<p class="se-sub">Pick a time that works for you. The booking page opens in a new tab.</p>' +
        '<a class="btn btn-primary" href="' + url + '" target="_blank" rel="noopener">' + esc(s.button || s.heading || "Book a Consultation") + svg("arrow") + '</a>' +
      '</div></div></section>';
  }
